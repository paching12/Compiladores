...
%token <sim> racionalnum var undef bltin WHILE IF ELSE PRINT op FOR
%type   <inst>  stmt asigna exp stmtlist cond while if end for
...
stmt:     exp  { code(pop1); }
	...
	| for '('exp end1';' cond ';' exp end1')' stmt end
		{	
		($1)[1]   =   (Inst)$6;	 //asignacion
		($1)[2]   =   (Inst)$8; //incremento
		($1)[3]   =   (Inst)$11;	//cuerpo de la iteracion
		($1)[4]   =   (Inst)$12; // termina si no se cumple "end"
		}
	|   '{'   stmtlist   '}' {   $$  =  $2;   }
        ;
